//! Paragraph-wrap golden tests.
//!
//! Mirrors `golden_inline.rs` / `golden_block.rs`: every fixture under
//! `tests/golden_wrap/` is an `*.in` / `*.out` pair, optionally with
//! an `*.config.toml` setting `[fmt] wrap = …`. Default config uses
//! `wrap = "keep"`.
...
#[test]
fn golden_wrap() {
    let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/golden_wrap");
