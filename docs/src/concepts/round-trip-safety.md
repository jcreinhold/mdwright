...
Some syntactically-equivalent rewrites are not applied. The clearest case: mdwright leaves a setext heading as-is rather
than converting it to ATX when the conversion would change the HTML id-anchor that external links point at. The cost of
round-trip safety is that the formatter sometimes declines a clean-up it could otherwise perform.
...
line of HTML. The fix lives in one of three places:

1. **Document recognition** misclassified a span: fix the document facts, not the formatter.
2. **A rewrite producer** proposed a stale or over-broad byte edit: fix the candidate owner/range or the verification
