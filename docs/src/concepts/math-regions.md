...
[`src/format/math.rs`](https://github.com/jcreinhold/mdwright/blob/main/src/format/math.rs) — is that the math scanner
runs **before** the `pulldown-cmark` event walk that builds the formatter's typed IR. The scanner produces a sorted
list of byte ranges; the IR builder consults this list when it would otherwise descend into a span and instead emits a
`NodeKind::Math` leaf whose payload is the verbatim source slice.

This means:

- Math is not "parsed" by mdwright. The exact source bytes — whitespace, casing, comment chars, trailing backslashes —
  pass through unchanged.
- The formatter cannot accidentally apply emphasis, escape, or wrap logic to math: those passes walk the IR tree, and
  math is a leaf they do not recurse into.
