...
- **Environments.** `\begin{NAME} … \end{NAME}` for any `NAME` matching `[A-Za-z][A-Za-z0-9*]*`, paired with a
...
The [stray-dollar](../rules/stray-dollar.md) lint flags lone dollar signs when this option is off, so authors migrating
from a dollar-delimited dialect catch the change.
...
A math environment whose start delimiter sits at column 1 of an otherwise-blank line is a block. The formatter emits
blocks with one blank line above and below, never indented inside a list item unless the source already indented it.
This avoids the canonical bug:
