...
| [`math/unbalanced-braces`](math/unbalanced-braces.md) | yes | no | no | `{` / `}` inside a math body do not balance; the pretty-printer falls back to verbatim emission for that region. |
...
| [`list-tightness-flipped`](list-tightness-flipped.md) | no | yes | no | list tightness from typed items disagrees with tightness from source bytes |
