...
`mdformat --check`. The multiplier scales with file count and core count; see [Performance](reference/performance.md)
for the measurement, host, and reproducer. Design choices that buy this are in
...
treated as a bug. Whitespace inside a paragraph may shift (`a  b` becomes `a b`), but word boundaries and the rendered
tree do not. Where the formatter cannot prove equivalence it refuses to rewrite; the [deviation table](deviations.md)
lists every exception with a reproducer.

**Configurable, preserve by default.** Source style choices — emphasis delimiters, list markers, thematic breaks,
link-destination angle brackets — pass through untouched. Canonicalisation is opt-in one knob at a time in
`.mdwright.toml`, or via `fmt.profile = "mdformat"` for mdformat-compatible spelling where verified rewrites preserve
the parsed document.
