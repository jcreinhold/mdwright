...
| `fmt.lists.continuation-indent` | "marker-width" \| "four-space" | `"marker-width"` | `none` | Continuation indentation for wrapped list-item paragraphs. `marker-width` aligns to the source marker width; `four-space` matches mdformat's list continuation spelling. The mdformat profile defaults this key to `four-space`. |
