...
[Writing a lint rule](lint-rules.md) describes: depend on `mdwright-document` and `mdwright-lint`, implement `LintRule`,
...
Architecture B is unlikely to ever become attractive: Rust would have to grow a stable ABI, which is not on any horizon,
and even then the cost-benefit against custom binaries would barely move.
...
please [open an issue](https://github.com/jcreinhold/mdwright/issues) describing the concrete adoption story. A
motivated maintainer behind a real need is the precondition for revisiting this.
