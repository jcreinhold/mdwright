...
//!   single-pass and block-snapped formatting via the same
//!   [`crate::Document`] / [`crate::format_range_with_checkpoints`]
...
//!   every safe [`crate::Fix`], plus a bundled
...
use crate::{
    CheckpointTable, Config, Diagnostic as MdwrightDiagnostic, Document, FmtOptions, LineIndex, RuleSet,
    Severity as MdwrightSeverity, format_range_with_checkpoints, rule_doc_url, stdlib,
};
...
impl LanguageServer for MdwrightLs {
    ...
    async fn did_open(&self, params: DidOpenTextDocumentParams) {
        ...
        let (text, version, line_index, rules) = {
        ...
        drop(state);
        self.lint_and_publish(uri, text, version, line_index, rules).await;
    }
    ...
    async fn code_action(&self, params: CodeActionParams) -> RpcResult<Option<CodeActionResponse>> {
        ...
        if diags.iter().any(|d| d.fix.as_ref().is_some_and(|f| f.safe)) {
            let parsed = Document::parse(&source);
            let (new_text, applied) = parsed.apply_safe_fixes(&diags);
...
impl MdwrightLs {
    async fn lint_and_publish(
        ...
        run_lint_pass(&self.client, &self.state, uri, text, version, line_index, rules).await;
    }
    ...
    async fn refresh_config_and_relint(&self) {
        ...
        for (uri, text, version, line_index) in to_relint {
            self.lint_and_publish(uri, text, version, line_index, Arc::clone(&rules))
                .await;
        }
    }
}
