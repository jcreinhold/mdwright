...
//!
//! External tools that want to ship a custom mdwright binary — one
//! that adds extra [`LintRule`](crate::LintRule)s on top of the
//! stdlib — depend on this module rather than the binary. The
//! [`run_with_rules`] function takes a fully-populated
//! [`RuleSet`](crate::RuleSet) and runs the standard CLI on top of
//! it: arg parsing, config discovery, output formatting, LSP, and
//! every other detail of the official binary.
//!
...
/// to rules with [`LintRule::is_default`](crate::LintRule::is_default),
/// `--rules all` selects every rule in it, and `--rules <name>` /
/// `--rules +<name>` reject names that are not present. The official
/// binary passes [`stdlib::all`] so every stdlib opt-in is selectable;
/// downstream binaries pass `stdlib::all()` plus their own
/// [`RuleSet::add`](crate::RuleSet::add)ed rules.
///
/// On unrecoverable error, prints `mdwright: error: <message>` to
/// stderr and returns `ExitCode::from(2)`. Callers wanting a different
/// error UX should bypass this function and call
/// [`Document::lint`](crate::Document::lint) /
/// [`Document::format`](crate::Document::format) directly.
///
...
fn run_lsp() -> Result<ExitCode> {
    ...
    runtime.block_on(crate::lsp::serve());
    Ok(ExitCode::SUCCESS)
}
