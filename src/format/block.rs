...
//! source-verbatim overlays (frontmatter, admonitions, and root-level
//! html / indented-code / verbatim-eligible paragraph), and dispatches
//! the remaining nodes through
//! [`TypedBlock::pretty`](crate::cm::block::TypedBlock::pretty).
//!
//! Math is a tree-level concept: the recogniser produces
//! `NodeKind::Math` leaves at parse time, so this module never touches
//! math directly. Per-construct serialisation logic lives next to each
//! typed value in `src/cm/block/*`. This module owns only the
//! document-shape decisions: how blocks are separated, when an overlay
//! short-circuits IR-driven emission, and the document-root tail
//! passes for link reference definitions and end-placed footnote
//! definitions.
