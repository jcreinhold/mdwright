...
[Architecture overview](https://jcreinhold.github.io/mdwright/extending/architecture.html)
on the docs site; this file is a map for someone reading the repo directly.

## Crate stack

mdwright is a virtual Cargo workspace under `crates/`. Each crate hides one kind of
knowledge; downstream crates depend only on layers below.
...
- `mdwright-document` — pulldown invocation behind a containment boundary; recognised
  Markdown facts with stable source coordinates. Every downstream crate reads `Document`,
  not pulldown events.
- `mdwright-config` — TOML schema and discovery walk.
- `mdwright-format` — verified byte-rewrite formatting. Identity by default; canonicalise
  and wrap families are gated by `semantically_equivalent`.
...
The deeper architecture notes live in `docs/architecture/` and are surfaced on the docs
site under the **Architecture** section:
...
- `parser-backend-audit.md`, `test-matrix.md`, `stability.md`,
  `latex-boundary-and-dependency-audit.md` — assorted release-gate evidence.

## See also

- [`CLAUDE.md`](CLAUDE.md) — repo-wide working agreements.
- [`CONTRIBUTING.md`](CONTRIBUTING.md) — PR conventions and MSRV-bump policy.
- [`docs/src/reference/public-api.md`](docs/src/reference/public-api.md) — every public
  Rust item, mapped to the crate that owns it.
