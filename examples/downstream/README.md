...
- `.pre-commit-config.yaml` — wires the `-system` hook variants against an `mdwright` already on `$PATH`. The
  integration test (`tests/downstream_integration.rs`) bypasses the `pre-commit` framework entirely and invokes the
  binary directly, so the test stays hermetic; the file documents the consumer-facing usage pattern.
