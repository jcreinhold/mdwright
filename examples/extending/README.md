...
This crate shows the supported pattern for adding a third-party lint rule to mdwright: depend on the component crates
and command package, implement [`mdwright_lint::LintRule`], and call [`mdwright::run_with_rules`] from your own `main`.
You ship one binary; it has the full mdwright UX (`check`, `fix`, `fmt`, `lsp`, `--rules`, JSON output, suppression
...
That's the whole binary. `run_with_rules` does the rest: clap parsing, config discovery, output formatting, the LSP
server, everything. Your rule is then selectable like any stdlib rule:
...
1. In `Cargo.toml`, add `mdwright = "0.1"`, `mdwright-document = "0.1"`, and `mdwright-lint = "0.1"` (or whatever
   version is current; the public extension API follows mdwright's [semver policy](../../docs/src/reference/semver.md)).
1. Copy `src/no_todo.rs` as a template; replace the rule's logic with whatever check you need.
1. Copy `src/main.rs` verbatim.
1. `cargo publish` (or just `cargo build --release` and distribute the binary directly).

Downstream users install your binary and run it exactly as they would the official `mdwright`.
