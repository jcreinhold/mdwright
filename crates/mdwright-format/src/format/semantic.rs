...
use mdwright_document::{ParseOptions, markdown_signature};
...
pub(crate) fn semantically_equivalent_with_options(source: &str, formatted: &str, parse_options: ParseOptions) -> bool {
    markdown_signature(source, parse_options) == markdown_signature(formatted, parse_options)
}
...
pub(crate) fn first_divergence_with_options(
    ...
    source_sig.first_divergence(&formatted_sig)
}
