...
//! document's meaning?" is the load-bearing question the runtime gate
//! and offline correctness suites must answer.
//!
...
pub(crate) fn canonical_events(src: CanonicalSource<'_>) -> Vec<CanonicalEvent> {
    ...
    for ev in parse::events(src, parse::options(mdwright_document::ParseOptions::default())) {
        match ev {
            Event::Start(tag) => {
...
/// True iff the two Markdown sources parse to the same canonical event
/// stream.
///
/// The runtime gate, property tests, the GFM-spec runner, and the
/// `fuzz_parse_format` oracle all route through this single definition.
///
