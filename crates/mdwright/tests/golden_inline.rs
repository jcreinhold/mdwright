//! Inline-content formatter golden tests.
//!
//! Each fixture is an `*.in` / `*.out` pair under
//! `tests/golden_inline/`. Mirrors `tests/golden_block.rs`: walk the
...
#[test]
fn golden_inline() {
    let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/golden_inline");
