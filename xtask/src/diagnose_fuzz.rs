...
/// from `Document::parse` / `Document::format_validated` (typically a
/// pulldown-cmark upstream bug — the fuzz target swallows these via
/// `catch_unwind`; here they propagate so the diagnostic doesn't
/// silently mis-report).
pub fn diagnose(artifact_path: &Path) -> Result<Diagnosis> {
    ...
    // equivalence. This is stricter than `Document::format_validated`
    // (which checks idempotence-on-mode, not source-vs-formatted) — the
    // diagnostic targets the same property the fuzz target asserts.
    let formatted = Document::parse(s).format(&opts);
